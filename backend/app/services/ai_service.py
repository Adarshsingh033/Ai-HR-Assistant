...
class ExtractResumeDataSchema(BaseModel):
    ...
    education: str = Field(
        ..., description="Education summaries"
    )

    gender: str = Field(
        ..., description="Gender of the candidates(Ex: Male or Female)"
    )

class MatchResultSchema(BaseModel):
    match_percentage: int = Field(..., description="Percentage match between 0 and 100", ge=0, le=100)
    match_explanation: str = Field(..., description="Explanation of why the candidate matches or doesn't match")

ollama_client = ChatOllama(model="gemma2:9b",temperature= 0.0, num_gpu=1)
